...
import { createRoomChat } from '../lib/actions';
...
export default function CreateRoomForm(
    ...
    const handleCreateRoom = async (event: any) => {
        ...
        const originalString = formData.roomname;
        const convertedString = originalString
            .toLowerCase() // Chuyển thành chữ thường
            .normalize('NFD') // Chuẩn hóa chuỗi Unicode để tách các ký tự diacritic (dấu) thành ký tự cơ bản và dấu
            .replace(/[\u0300-\u036f]/g, '') // Loại bỏ tất cả các ký tự diacritic (dấu)
            .split(' ')    // Tách chuỗi thành mảng các từ
            .join('-');    // Nối các từ lại với dấu gạch ngang
        formData.id = convertedString
        console.log(formData)
        await createRoomChat(formData, formData.id)
        ...
        <div id="crud-modal" aria-hidden="true" className="flex overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div className="relative p-4 w-full max-w-md max-h-full">
                ...
                <div className="relative bg-gray-700 rounded-lg shadow dark:bg-gray-700">
                    ...
                    <form onSubmit={handleCreateRoom} className="p-4 md:p-5">
                        <div className="grid gap-4 mb-4 grid-cols-2">
                            <div className="col-span-2">
                                <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Room name</label>
                                <input type="text" name="roomname" id="roomname" value={formData.roomname} onChange={handleChange} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Type room name" />
                            </div>
                            <div className="col-span-2 sm:col-span-1">
                                <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Image</label>
                                <input type="text" name="img" id="img" value={formData.img} onChange={handleChange} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Url" />
                            </div>
                            <div className="col-span-2">
